...
@click.option(
    ...
    "sesh_profile_name",
    type=str,
    help="The seshkit config profile to write/update",
)
...
def interactive_profile_editor(profile: dict) -> dict:
    ...
    prof["service_creds_path"] = _credspath = click.prompt(
        f"What is the file path for service credentials?",
        default=_dv,
        type=click.Path(),
    )
    ...
    prof["service_profile"] = click.prompt(
        "Which account profile?",
        type=click.Choice(_credprofiles),
        default=_dv,
    )
    ...
    prof["output_bucket"] = click.prompt(
        "What is the name/id of the default output bucket? e.g. 'my-output-bucket'",
        type=str,
        default=_dv,
    )
